...
<body>
        ...
        <div class="dashboard">
          ...
          <div class="dashboard-col ops-col">
            ...
            <section class="panel new-panel">
              ...
              <div class="panel-body">
                <textarea id="projectDescription" rows="3" placeholder="Describe the project — e.g. a dashboard for tracking A-share holdings with a React frontend and a FastAPI backend."></textarea>
                <button id="plan" class="launch-button" type="button">Generate plan</button>
                <div id="planResult" class="plan-result" hidden></div>
                <datalist id="feOptions">
                  <option value="None"></option>
                  <option value="React + Vite"></option>
                  <option value="React + Vite + TypeScript"></option>
                  <option value="Next.js"></option>
                  <option value="Next.js + TypeScript"></option>
                  <option value="Vue 3 + Vite"></option>
                  <option value="Vue 3 + Vite + TypeScript"></option>
                  <option value="Svelte + Vite"></option>
                  <option value="SvelteKit"></option>
                  <option value="Astro"></option>
                  <option value="Angular"></option>
                  <option value="SolidJS"></option>
                  <option value="Plain HTML + CSS + JS"></option>
                  <option value="React Native"></option>
                  <option value="Flutter"></option>
                  <option value="Electron"></option>
                </datalist>
                <datalist id="beOptions">
                  <option value="None"></option>
                  <option value="Node.js + Express"></option>
                  <option value="Node.js + Express + TypeScript"></option>
                  <option value="Node.js + NestJS"></option>
                  <option value="Node.js + Fastify"></option>
                  <option value="Python + FastAPI"></option>
                  <option value="Python + Flask"></option>
                  <option value="Python + Django"></option>
                  <option value="Go + net/http"></option>
                  <option value="Go + Gin"></option>
                  <option value="Rust + Axum"></option>
                  <option value="Java + Spring Boot"></option>
                  <option value="C# + ASP.NET Core"></option>
                  <option value="PHP + Laravel"></option>
                </datalist>
                <datalist id="dbOptions">
                  <option value="None"></option>
                  ...
                  <option value="Supabase (PostgreSQL)"></option>
                  <option value="Firebase Firestore"></option>
                  <option value="PlanetScale (MySQL)"></option>
                  <option value="Turso (LibSQL)"></option>
                </datalist>
                <div class="new-grid">
                  <input id="newName" type="text" placeholder="my-new-project">
                </div>
                <div class="new-actions">
                  <button id="create" class="secondary" type="button">Create</button>
                  <button id="createOpen" class="primary" type="button" disabled>Create &amp; Open</button>
                </div>
  ...
  <script>
    ...
    async function createProjectAction(launch) {
      ...
      if (launch && !currentPlan) { showToast('Generate a plan first', 'err'); return; }

      setStatus(launch ? 'Scaffolding & opening\u2026' : 'Creating project\u2026');
      const payload = {
        name,
        compliance,
        plan: currentPlan || null,
        launch: Boolean(launch),
        tool: launch && currentPlan ? currentPlan.tool : null,
      };
      const data = await post('/api/new', payload);
      if (data.error) {
        setStatus(data.error, 'err');
        showToast(data.error, 'err');
        return;
      ...
      if (data.launchError) {
        setStatus(message + ' — ' + data.launchError, 'err');
        showToast(message + ' — ' + data.launchError, 'err');
      } else {
    ...
    document.getElementById('create').addEventListener('click', () => createProjectAction(false));
    document.getElementById('createOpen').addEventListener('click', () => createProjectAction(true));
    ...
    function renderPlan(data) {
      ...
      currentPlan = {
        ...
        ux: plan.ux || '',
        frontend: { stack: (plan.frontend && plan.frontend.stack) || 'None', reason: (plan.frontend && plan.frontend.reason) || '' },
        backend: { stack: (plan.backend && plan.backend.stack) || 'None', reason: (plan.backend && plan.backend.reason) || '' },
        database: { name: (plan.database && plan.database.name) || 'None', reason: (plan.database && plan.database.reason) || '' },
        tool: plan.tool || (allTools[0] ? allTools[0].id : ''),
        toolReason: plan.toolReason || '',
        nextSteps: Array.isArray(plan.nextSteps) ? plan.nextSteps : [],
      };

      const toolOptions = allTools.map((item) =>
        '<option value="' + esc(item.id) + '"' + (item.id === currentPlan.tool ? ' selected' : '') + '>' + esc(item.label) + '</option>',
      ).join('');

      planResult.innerHTML =
        '<p class="plan-summary">' + esc(currentPlan.summary) + '</p>' +
        '<div class="plan-block">' +
          '<div class="plan-label">UI / UX</div>' +
          '<div class="plan-value">' + esc(currentPlan.ux) + '</div>' +
        '</div>' +
        '<div class="plan-block">' +
          '<label class="plan-label" for="planFrontend">Frontend stack</label>' +
          '<input id="planFrontend" class="plan-edit" data-field="frontend.stack" list="feOptions" placeholder="Choose or type a stack" value="' + esc(currentPlan.frontend.stack) + '">' +
        '</div>' +
        '<div class="plan-block">' +
          '<label class="plan-label" for="planBackend">Backend stack</label>' +
          '<input id="planBackend" class="plan-edit" data-field="backend.stack" list="beOptions" placeholder="Choose or type a stack" value="' + esc(currentPlan.backend.stack) + '">' +
        '</div>' +
        '<div class="plan-block">' +
          '<label class="plan-label" for="planDatabase">Database</label>' +
          '<input id="planDatabase" class="plan-edit" data-field="database.name" list="dbOptions" placeholder="Choose or type a database" value="' + esc(currentPlan.database.name) + '">' +
        '</div>' +
        '<div class="plan-block">' +
          '<label class="plan-label" for="planTool">Open with</label>' +
          '<select id="planTool" class="plan-edit" data-field="tool">' + toolOptions + '</select>' +
        '</div>' +
        (currentPlan.nextSteps.length
          ? '<div class="plan-block">' +
              '<div class="plan-label">Next steps</div>' +
              '<ul class="plan-next">' + currentPlan.nextSteps.map((step) => '<li>' + esc(step) + '</li>').join('') + '</ul>' +
            '</div>'
          : '');

      planResult.hidden = false;
      createOpen.disabled = false;
      if (currentPlan.name && !document.getElementById('newName').value.trim()) {
        document.getElementById('newName').value = currentPlan.name;
      }
      if (data.mode === 'fallback') {
        setStatus(data.warning || 'Used offline recommendations', 'ok');
        showToast(data.warning || 'Used offline recommendations');
      } else {
        setStatus('Plan ready — adjust any stack, database, or tool, then Create', 'ok');
      }
    }

    planButton.addEventListener('click', async () => {
      const description = projectDescription.value.trim();
      if (!description) { showToast('Describe the project first', 'err'); return; }
      setStatus('Generating plan\u2026');
      planButton.disabled = true;
      try {
        const data = await post('/api/plan', { description });
        if (data.error) {
          setStatus(data.error, 'err');
          showToast(data.error, 'err');
        } else {
        ...
        setStatus('Could not generate a plan', 'err');
        showToast('Plan request failed', 'err');
      } finally {
        planButton.disabled = false;
      }
    });

    function syncPlanField(event) {
      if (!currentPlan) return;
      const el = event.target && event.target.closest ? event.target.closest('[data-field]') : null;
      if (!el) return;
      const field = el.getAttribute('data-field');
      const value = el.value;
      const parts = field.split('.');
      if (parts.length === 2) {
        currentPlan[parts[0]] = currentPlan[parts[0]] || {};
        currentPlan[parts[0]][parts[1]] = value;
      } else {
        currentPlan[parts[0]] = value;
      }
    }

    planResult.addEventListener('input', syncPlanField);
    planResult.addEventListener('change', syncPlanField);
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === 'POST' && url.pathname === '/api/new') {
      ...
      try {
        ...
        const files = [];
        if (plan) {
          const tool = detectTools().find((t) => t.id === plan.tool);
          if (tool) plan.toolLabel = tool.label;
          plan.name = path.basename(target);
          files.push(...writeScaffold(target, plan));
        }

        let launched = false;
        let launchError = null;
        let openedToolLabel = null;
        if (body.launch) {
          const tool = detectTools().find((t) => t.id === (body.tool || (plan && plan.tool)));
          if (containerMode()) {
            launchError = 'Launching desktop tools is disabled in Docker mode.';
          } else if (!tool) {
            launchError = 'The suggested tool is not installed on this machine.';
          } else {
            const cfg = getConfig();
            const profile =
              cfg.defaultProfile && cfg.profiles && cfg.profiles[cfg.defaultProfile]
                ...
                : null;
            try {
              launchToolWeb(tool, target, profile);
              launched = true;
              openedToolLabel = tool.label;
              recordTelemetry(path.basename(target), tool.id);
            } catch (error) {
        ...
        const message =
          'Created ' + target +
          (files.length ? ' with ' + files.length + ' scaffold file' + (files.length === 1 ? '' : 's') : '') +
          (launched ? ' and opened ' + openedToolLabel : '');

        json(res, 200, { ok: true, target, files, launched, launchError, message });
      } catch (error) {
    ...
    if (req.method === 'POST' && url.pathname === '/api/plan') {
      ...
      if (!description) {
        json(res, 400, { error: 'Describe the project first.' });
        return;
      }
      try {
        const result = await buildPlan(description);
        json(res, 200, result);
      } catch (error) {
        json(res, 500, { error: error.message });
      }
